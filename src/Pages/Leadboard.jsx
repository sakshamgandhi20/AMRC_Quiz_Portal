...
const Leaderboard = () => {
  ...
  useEffect(() => {
  if (!config || !config.sheetUrl) return; // ⛔ Avoid fetch if config is not ready
            ...
            <tbody>
              {entries.map((entry, index) => (
                <tr key={index} className="text-center">
                  <td className="py-2 text-black px-4 border">{entry.name}</td>
                  <td className="py-2 px-4 text-black border">{entry.location}</td>
